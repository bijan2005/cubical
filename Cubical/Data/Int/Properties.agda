...
sucPred : ∀ (i : ℤ) → sucInt (predInt i) ≡ i
predSuc : ∀ (i : ℤ) → predInt (sucInt i) ≡ i

discreteInt : discrete ℤ
isSetInt : isSet ℤ

addition of ℤ is defined _+_ : ℤ → ℤ → ℤ
...
isEquivAddInt : (m : ℤ) → isEquiv (λ n → n + m)
...
open import Cubical.Data.Nat as ℕ using (ℕ; zero; suc; discreteℕ)
...
applysgn : Bool → ℕ → ℤ
applysgn true n = pos n
applysgn false zero = pos zero
applysgn false (suc n) = negsuc n

sgn-applysgn : (b : Bool) (n : ℕ) → sgn (applysgn b (suc n)) ≡ b
sgn-applysgn true n = refl
sgn-applysgn false n = refl

applysgn-idem : (a b : Bool) (n : ℕ) → applysgn a (abs (applysgn b n)) ≡ applysgn a n
applysgn-idem true true n = refl
applysgn-idem true false zero = refl
applysgn-idem true false (suc n) = refl
applysgn-idem false true zero = refl
applysgn-idem false true (suc n) = refl
applysgn-idem false false zero = refl
applysgn-idem false false (suc n) = refl
...
_+pos_ : ℤ → ℕ → ℤ
m +pos 0 = m
m +pos (suc n) = sucInt (m +pos n)

_+negsuc_ : ℤ → ℕ → ℤ
m +negsuc 0 = predInt m
m +negsuc (suc n) = predInt (m +negsuc n)
...
negsuc0+ : ∀ n → predInt n ≡ negsuc 0 + n
...
isEquivAddInt : (m : ℤ) → isEquiv (λ n → n + m)
isEquivAddInt = subst (λ add → (m : ℤ) → isEquiv (λ n → add n m)) +'≡+ isEquivAddInt'
...
inj-+m : ∀ {m n o} → n + m ≡ o + m → n ≡ o
inj-+m {m} {n} {o} p = sym (plusMinus m n) ∙ cong (_- m) p ∙ plusMinus m o

inj-m+ : ∀ {m n o} → m + n ≡ m + o → n ≡ o
inj-m+ {m} {n} {o} p = inj-+m {m} {n} {o} (+-comm n m ∙ p ∙ +-comm m o)

_*_ : ℤ → ℤ → ℤ
m * n = applysgn (not (sgn m ⊕ sgn n)) (abs m ℕ.* abs n)

*-comm : ∀ m n → m * n ≡ n * m
*-comm m n = cong₂ (λ m n → applysgn (not m) n) (⊕-comm (sgn m) (sgn n)) (ℕ.*-comm (abs m) (abs n))

*-identityˡ : ∀ n → 1 * n ≡ n
*-identityˡ (negsuc n) = transport (λ i → negsuc (ℕ.*-identityˡ n (~ i)) ≡ negsuc n) refl
*-identityˡ (pos n) = transport (λ i → pos (ℕ.*-identityˡ n (~ i)) ≡ pos n) refl

*-identityʳ : ∀ n → n * 1 ≡ n
*-identityʳ n = transport (cong (_≡ n) (*-comm 1 n)) (*-identityˡ n)
...
*-zeroʳ : ∀ n → n * 0 ≡ 0
*-zeroʳ n = transport (cong (_≡ 0) (*-comm 0 n)) (*-zeroˡ n)

private
  lemma : ∀ m n o → o ℕ.+ (n ℕ.+ m ℕ.* suc n) ℕ.* suc o
                  ≡ o ℕ.+ n ℕ.* suc o ℕ.+ m ℕ.* suc (o ℕ.+ n ℕ.* suc o)
  lemma m n o =
    o ℕ.+ (n ℕ.+ m ℕ.* suc n) ℕ.* suc o                 ≡⟨ cong (o ℕ.+_) (sym (ℕ.*-distribʳ n (m ℕ.* suc n) (suc o))) ⟩
    o ℕ.+ (n ℕ.* suc o ℕ.+ m ℕ.* suc n ℕ.* suc o)       ≡⟨ ℕ.+-assoc o (n ℕ.* suc o) (m ℕ.* suc n ℕ.* suc o) ⟩
    o ℕ.+ n ℕ.* suc o ℕ.+ m ℕ.* suc n ℕ.* suc o         ≡⟨ cong (λ m → o ℕ.+ n ℕ.* suc o ℕ.+ m) (sym (ℕ.*-assoc m (suc n) (suc o))) ⟩
    o ℕ.+ n ℕ.* suc o ℕ.+ m ℕ.* suc (o ℕ.+ n ℕ.* suc o) ∎

  *-assocₛ : ∀ m n o → (m * n) * o ≡ m * (n * o)
  *-assocₛ (pos zero) n o = cong (_* o) (*-zeroˡ n) ∙ *-zeroˡ o ∙ sym (*-zeroˡ (n * o))
  *-assocₛ m (pos zero) o = cong (_* o) (*-zeroʳ m) ∙ *-zeroˡ o ∙ sym (cong (m *_) (*-zeroˡ o) ∙ *-zeroʳ m)
  *-assocₛ m n (pos zero) = *-zeroʳ (m * n) ∙ sym (cong (m *_) (*-zeroʳ n) ∙ *-zeroʳ m)
  *-assocₛ (negsuc a) (negsuc b) (pos (suc c)) = cong (λ m → pos (suc m)) (lemma a b c)
  *-assocₛ (negsuc a) (pos (suc b)) (negsuc c) = cong (λ m → pos (suc m)) (lemma a b c)
  *-assocₛ (pos (suc a)) (pos (suc b)) (pos (suc c)) = cong (λ m → pos (suc m)) (lemma a b c)
  *-assocₛ (pos (suc a)) (negsuc b) (negsuc c) = cong (λ m → pos (suc m)) (lemma a b c)
  *-assocₛ (negsuc a) (negsuc b) (negsuc c) = cong negsuc (lemma a b c)
  *-assocₛ (negsuc a) (pos (suc b)) (pos (suc c)) = cong negsuc (lemma a b c)
  *-assocₛ (pos (suc a)) (negsuc b) (pos (suc c)) = cong negsuc (lemma a b c)
  *-assocₛ (pos (suc a)) (pos (suc b)) (negsuc c) = cong negsuc (lemma a b c)

*-assoc : ∀ m n o → m * (n * o) ≡ (m * n) * o
*-assoc m n o = sym (*-assocₛ m n o)
