...
effective : (Rprop : isPropValued R) (Requiv : IsEquivalence R) (a b : A) → [ a ] ≡ [ b ] → R a b
effective {A = A} {R = R} Rprop (isequivalence reflexive (ispartialeq symmetric transitive)) a b p = transport aa≡ab reflexive
  where
    helper : A / R → hProp _
    helper =
      rec isSetHProp (λ c → (R a c , Rprop a c))
                     (λ c d cd → Σ≡Prop (λ _ → isPropIsProp)
                                        (hPropExt (Rprop a c) (Rprop a d)
                                                  (λ ac → transitive ac cd) (λ ad → transitive ad (symmetric cd))))

    aa≡ab : R a a ≡ R a b
    aa≡ab i = helper (p i) .fst
{-
isEquivRel→isEffective : isPropValued R → IsEquivalence R → isEffective R
...
-}
discreteSetQuotients : Discrete A → isPropValued R → IsEquivalence R → Decidable R → Discrete (A / R)
