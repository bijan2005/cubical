...
compIsEquiv : {f : A → B} {g : B → C} → isEquiv f → isEquiv g → isEquiv (g ∘ f)
compIsEquiv {f = f} {g} fe ge = isoToIsEquiv (compIso (equivToIso (f , fe)) (equivToIso (g , ge)))
...
-- Reasoning on equivalence

infixr 1 _≃⟨_⟩_ _≃⟨⟩_
infixr 2 _■

_≃⟨_⟩_ : (T : Type ℓ) → T ≃ A → A ≃ B → T ≃ B
T ≃⟨ f ⟩ g = compEquiv f g

_≃⟨⟩_ : (T : Type ℓ) → T ≃ A → T ≃ A
T ≃⟨⟩ f = f
